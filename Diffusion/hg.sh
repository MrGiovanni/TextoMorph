...
python3 train.py dataset.name=kidney_tumor_train dataset.fold=$fold dataset.data_root_path=$datapath dataset.label_root_path=$tumorlabel dataset.dataset_list=['kits_tumor_data_early_fold'] dataset.uniform_sample=False model.results_folder_postfix="kidney_early_tumor_fold0" model.vqgan_ckpt=$vqgan_ckpt
