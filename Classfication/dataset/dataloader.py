...
def get_loader(args):
    train_transforms = Compose(
        [
            LoadImaged_totoalseg(keys=["image"], map_type=args.map_type,location=args.location),
    ...
    # import pandas as pd
    # import random
    # healthy_id=pd.read_csv('/ccvl/net/ccvl15/cliu234/projects/DiffTumor/STEP3.SegmentationModel/dataset/dataset_list/JHH_healthy.csv')
    # healthy_id=healthy_id[healthy_id['healthy']==1]['jhh_id'].values
    # with open('/ccvl/net/ccvl15/cliu234/projects/DiffTumor/STEP3.SegmentationModel/utils/jhh_small_tumor_ct_pnet.txt','r') as f:
    #     pnet_small=f.readlines()
    #     pnet_small=[i.strip() for i in pnet_small]
    ########################
    ...
    if args.stage == 'train':
        ...
        for item in args.dataset_list:
            for line in open(os.path.join(args.data_txt_path,item +'.txt')):
                ...
                #############################
                # if name in healthy_id:
                #     if random.random() > 0.5:
                #         print(f'prev_name:{name}')
                #         name = random.choice(pnet_small)
                #         print(f'after_name:{name}')
                #############################       
        ...
        data_dicts_train=data_dicts_train
        ...
        train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=(train_sampler is None), num_workers=args.num_workers, #args.num_workers
